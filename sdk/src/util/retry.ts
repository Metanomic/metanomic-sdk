// forked implementation of https://github.com/JustinBeckwith/retry-axios
// to provide a more simplified version
import axios, {
  AxiosError,
  AxiosInstance,
  AxiosRequestConfig,
...
export function attach(instance: AxiosInstance) {
  instance.interceptors.response.use(onFulfilled, onError);
}
...
function onError(err: AxiosError) {
  ...
  config.retry = Number.isInteger(config.retry) ? config.retry : 3;
  config.retryDelay =
    Number.isInteger(config.retryDelay) ? config.retryDelay : 100;
  ...
  config.noResponseRetries =
    Number.isInteger(config.noResponseRetries) ? config.noResponseRetries : 2;
  config.checkRetryAfter =
    typeof config.checkRetryAfter === 'boolean' ? config.checkRetryAfter : true;
  config.maxRetryAfter =
    Number.isInteger(config.maxRetryAfter) ? config.maxRetryAfter : 60000 * 5;
  ...
  const onBackoffPromise = new Promise((resolve, reject) => {
    ...
    if (delay === 0) {
      ...
      if (Number.isInteger(config.maxRetryDelay)) {
        delay = Math.min(delay, config.maxRetryDelay || 0);
      }
    }
